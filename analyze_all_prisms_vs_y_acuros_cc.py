...
for filename in prism_data_filenames:
    ...
    if int(lp_data) in index_bounds.keys():
        local_bounds = index_bounds[int(lp_data)]
        prism_data = prism_data[local_bounds[0]: local_bounds[1],:]
    ...
    #cc = abs(np.corrcoef(sub_position, sub_index)[0,1])
    result = linregress(sub_position, sub_index)
    #cc = result.pvalue
    #cc_err = 0
    cc = (result.slope * np.std(sub_position) / np.std(sub_index)) ** 2
    cc_err = (result.stderr * np.std(sub_position) / np.std(sub_index)) * 2
    cc = np.std(sub_index)
    cc_err = 0
    if metadata[2].split('.')[0] == "CT":
        control_cc.append(cc)
        control_cc_err.append(cc_err)
        int_cc.append(lp_data)
        #data_2 = sub_index
        #pos_2 = sub_position
        #continue
    else:
        calibrated_cc.append(cc)
        calibrated_cc_err.append(cc_err)

    #plt.scatter(np.linspace(0, len(sub_index) - 1, len(sub_index)), sub_index, s=3)
...
#plt.scatter(int_cc, control_cc, s=20)
#plt.scatter(int_cc, calibrated_cc, s=20)
plt.errorbar(int_cc, control_cc, control_cc_err, fmt='o')
plt.errorbar(int_cc, calibrated_cc, calibrated_cc_err, fmt='o')
