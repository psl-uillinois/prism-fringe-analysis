...
# Selectable by user
input_data_dir = "20221130_Prisms"
...
for filename in prism_data_filenames:
    ...
    # if "080int" not in filename:
    #    continue
    #plt.imshow(prism_data, cmap='hot')
    #plt.show()
    #exit(0)
    ...
    if len(int_data) > 1:
        int_data = [int_data[0]]
    if len(int_data) != 1:
        print("Warning: " + filename + " has unreadable `int` metadata")
        continue
    ...
    #if lp_data == 130:
    #    prism_data = prism_data[60:210]

    # Pass to fringe analysis program
    fringe_spacing = get_fringe_spacing(prism_data, physical_size[0] / np.shape(prism_data)[1], physical_size[1] / np.shape(prism_data)[0], prom=0.005, intensity=int_data)
    ...
    if convert_to_index:
        ...
        print(f"{filename}: n = {calculated_index}")
